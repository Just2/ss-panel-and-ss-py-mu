...
install_ss_panel_mod_v3(){
	...
	num=$1
	if [ "${num}" != "1" ]; then
  	  wget -c --no-check-certificate https://raw.githubusercontent.com/mmmwhy/ss-panel-and-ss-py-mu/master/lnmp1.3.zip && unzip lnmp1.3.zip && rm -rf lnmp1.3.zip && cd lnmp1.3 && chmod +x install.sh && ./install.sh lnmp
	fi
	cd /home/wwwroot/
